...
export default function LogSheetForm({ tripId, driverId, initialValues, onSuccess }: LogSheetFormProps) {
  ...
  const onSubmit = async (values: LogSheetFormValues) => {
    try {
      ...
      form.setError('root', { type: 'server', message: 'Failed to create log sheet.' });
    }
  };
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-6">
        ...
        <Button type="submit" disabled={isSubmitting}>{isSubmitting ? 'Creating...' : 'Create Log Sheet'}</Button>
      </form>
    </Form>
  );
}
